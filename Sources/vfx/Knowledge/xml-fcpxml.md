...
related: [export-rendering-media-encoder, automation, aaf-interchange, otio-interchange, premiere-to-resolve-handoff-guide]
sources: [
  "FCP7 XML specification (legacy but stable)",
  "Production testing: Premiere 25.x",
  "Community tools (fcpxml-parser, etc.)",
  "DaVinci Resolve manual (XML/AAF conform), 2026-07"
]
confidence: high
...
## Sending to DaVinci Resolve Specifically

**Added 2026-07.** Resolve treats an imported XML as a **conform**: it relinks every referenced clip against media already present in (or added to) the Media Pool by filename/reel/timecode, not a self-contained package — source media must be delivered to the Resolve machine separately.

**Verified gotchas:**
- Resolve's XML importer always splits stereo audio into two mono tracks (same as its AAF importer) — fix via Clip Attributes → Dual Mono → Stereo after import.
- A clip manually resized in Premiere via **Scale to Frame Size** carries a transform that doesn't translate correctly — right-click → **Set to Frame Size** instead, before export. On the Resolve side, set Settings → Image Scaling → Input Scaling → **Center crop with no resizing**.
- Nested sequences and multicam sequences are unreliable across this interchange — flatten to a single cut/track structure before export where possible.
- **Titles, Essential Graphics, and MOGRTs never transfer** — see "Sending to DaVinci Resolve Specifically" below and `premiere-to-resolve-handoff-guide.md` for the required pre-render workaround.

**Where XML fits among the three interchange options:** community consensus as of 2026 is that FCP7 XML remains the most robust of the three formats (XML / AAF / OTIO) for pure editorial-structure handoffs specifically — AAF from Premiere is reported as inconsistent beyond a simple cutlist, and OTIO (Premiere-side still Beta as of this writing) is younger and rougher at the edges for this specific path. Reach for AAF only when Fairlight-side audio mixing genuinely needs the sample-accurate handles it carries; reach for OTIO when the destination is a scripted pipeline step rather than a human-driven roundtrip. See `aaf-interchange.md` and `otio-interchange.md` for the full comparison, and `premiere-to-resolve-handoff-guide.md` for the decision matrix.
...
- Blackmagic Design DaVinci Resolve manual — XML/AAF conform chapter
- Studio Network Solutions, MASV — Premiere↔Resolve roundtrip workflow guides (2025-2026)
