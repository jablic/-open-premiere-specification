...
## Discovering Available QE Methods (Reflection)

**Added 2026-07.** Since QE is entirely undocumented, don't guess method names blind — QE objects expose a reflection API that lists what's actually callable on a given build:

```javascript
app.enableQE();
var qeSeq = qe.project.getActiveSequence();
// Lists the real method names available on this object, this Premiere version.
var methodNames = qeSeq.reflect.methods; // property, not a function call, on most builds
```

`qe.reflect.methods` (or `<object>.reflect.methods`, exact placement varies by build/tool) is the authoritative way to check whether a plausible-sounding method (`removeCaptionTrack`, `removeVideoTrack`, etc.) actually exists on THIS Premiere version before calling it — cheaper and more reliable than trial-and-error `try/catch` probing across a dozen guessed names. `ExtendScript Developer Tools` (or similar debugger) can also enumerate this interactively.

**Confirmed via QE (naming symmetry, not exhaustively verified across all versions):** `removeVideoTrack()` and `removeAudioTrack()` exist on the QE sequence object for track removal — a real, working escape hatch when the public DOM has no track-removal API at all (`Sequence` in the documented DOM has no `removeTrack`/`removeVideoTrack`/etc.). Given this naming pattern, and that QE already mirrors the DOM's `getCaptionTrackCount()`/`getCaptionTrackAt()` for reading captions, a `removeCaptionTrack()`-shaped method is plausible but NOT confirmed in official sources — reflect on the actual object (see above) before relying on it, and always verify success by re-reading the relevant count afterward rather than trusting a truthy return from an undocumented call.

**Production pattern for a defensive undocumented-API call with real verification:**
```javascript
function tryRemoveViaQE(qeSeq, candidateNames, getCountFn) {
    var before = getCountFn(qeSeq);
    if (!before) return { removed: 0, method: null };
    for (var i = 0; i < candidateNames.length; i++) {
        var name = candidateNames[i];
        if (typeof qeSeq[name] !== "function") continue;
        try {
            for (var idx = before - 1; idx >= 0; idx--) { qeSeq[name](idx); } // highest index first
            var after = getCountFn(qeSeq); // fresh read, don't trust the call's own return value
            if (after < before) return { removed: before - after, method: name };
        } catch (e) { /* try next candidate */ }
    }
    return { removed: 0, method: null };
}
```
Falls back gracefully (report "not available on this version", don't silently no-op) when no candidate works — same "never let an API gap look like success" discipline as the rest of this file.
